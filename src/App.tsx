...
export default function MeseIkpaV3() {
  ...
  const fetchSheetData = async () => {
    ...
    try {
      ...
      // OFFSET: Blok 7 kolom berulang (C-I, J-P, dst)
  ...
  return (
    <div className="flex h-screen bg-slate-50 overflow-hidden font-sans text-slate-900">
      ...
      {/* SIDEBAR */}
      <aside className={`${sidebarOpen ? 'w-64' : 'w-20'} bg-[#0F172A] transition-all duration-300 flex flex-col shadow-2xl`}>
        ...
        <nav className="flex-1 p-4 space-y-2">
          <button className="w-full flex items-center p-3 rounded-2xl bg-blue-600 text-white shadow-lg shadow-blue-600/30">
            <LayoutDashboard size={20}/> 
            {sidebarOpen && <span className="ml-3 text-[11px] font-bold uppercase tracking-widest">Dashboard Utama</span>}
          </button>
        </nav>
      </aside>
      ...
      <main className="flex-1 flex flex-col min-w-0">
        <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-8 shadow-sm z-10">
          <button onClick={() => setSidebarOpen(!sidebarOpen)} className="p-2 hover:bg-slate-100 rounded-xl text-slate-500">
            <Menu size={20} />
          </button>
          <div className="flex items-center gap-4">
             <div className="flex gap-1 p-1 bg-slate-100 rounded-xl shadow-inner">
               {[1, 2, 3, 4].map(tw => (
                 <button 
                    ...
                    className={`px-5 py-1.5 rounded-lg text-[10px] font-black uppercase transition-all ${selectedTW === tw ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-400'}`}
                 >
                    TW {tw}
                 </button>
               ))}
             ...
             <button 
                ...
                className="px-5 py-2 bg-blue-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest shadow-lg active:scale-95 transition-all"
             >
        ...
        <div className="flex-1 overflow-auto p-8 bg-[#F8FAFC]">
          <div className="max-w-7xl mx-auto space-y-8 animate-in">
            ...
            <div className="bg-[#0F172A] rounded-[2.5rem] p-12 text-white relative overflow-hidden shadow-2xl border border-white/5">
               ...
               <p className="text-blue-400 text-[10px] font-black uppercase tracking-[0.4em] mt-3 opacity-80">
                  Otomasi Kertas Kerja SAKTI • Triwulan {selectedTW}
               </p>
            </div>
                ...
                !item.isStarted ? (
                  ...
                  <div key={idx} className="bg-white p-8 rounded-[3rem] border border-slate-200 shadow-sm hover:shadow-2xl transition-all duration-500">
                    <div className="flex justify-between items-start mb-8">
                      <div className={`p-4 rounded-2xl shadow-sm ${item.isSuccess ? 'bg-emerald-50 text-emerald-500' : 'bg-rose-50 text-rose-500'}`}>
                        <TrendingUp size={22}/>
                      </div>
                    ...
                    <div className="mb-8">
                      <div className="flex justify-between items-end mb-3 font-black italic">
                        ...
                        <span className={`text-[9px] uppercase px-3 py-1 rounded-full ${item.isSuccess ? 'bg-emerald-50 text-emerald-600' : 'bg-rose-50 text-rose-600'}`}>
                      ...
                      <div className="w-full h-2.5 bg-slate-100 rounded-full overflow-hidden shadow-inner p-0.5">
                        <div 
                          className={`h-full rounded-full transition-all duration-1000 ${item.isSuccess ? 'bg-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.4)]' : 'bg-rose-500 shadow-[0_0_10px_rgba(244,63,94,0.4)]'}`} 
                          style={{ width: `${Math.min(item.pReal, 100)}%` }}
                        ></div>
                      </div>
                    </div>
      ...
      <style dangerouslySetInnerHTML={{ __html: `
        .animate-in { animation: fade-in 0.8s cubic-bezier(0.4, 0, 0.2, 1) forwards; } 
